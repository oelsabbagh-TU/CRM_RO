function save_results(mdict::Dict, EOM::Dict, ADMM::Dict, results::Dict, data::Dict, agents::Dict, scenario_overview_row::DataFrameRow,sens, zones::Vector{String})
    ...
    # Optional period map sidecar — adds Day and DayOfYear columns when present.
    period_map_path = joinpath(home_dir, "Input", "period_map.csv")
    period_map = isfile(period_map_path) ? CSV.read(period_map_path, DataFrame) : nothing

    for z in zones
        zone_df = DataFrame(Timestep = timesteps)
        if period_map !== nothing && nrow(period_map) == nT
            zone_df[!, "Day"] = period_map[!, "Day"]
            zone_df[!, "DayOfYear"] = period_map[!, "DayOfYear"]
        end
