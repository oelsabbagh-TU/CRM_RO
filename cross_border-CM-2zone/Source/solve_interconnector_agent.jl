function solve_interconnector_agent!(mod::Model)
    ...
    # Objective (rebuilt each iteration because λ_all, g_bar_all, ρ_all change)
    ...
    # -------------------------------------------------------------
    # redefining constraints that depend on updated parameters
    # -------------------------------------------------------------
    # border_limit, net_pos and sys_balance are data-invariant (they depend
    # only on BRANCHES / topology), so they are NOT rebuilt.
