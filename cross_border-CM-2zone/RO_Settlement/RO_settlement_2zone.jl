...
# ─────────────────────────────────────────────────────────────────────────────
# Reliability-Options settlement analysis — 2-zone variant
#
# Layout: cross_border-CM-2zone/RO_Settlement/RO_settlement_2zone.jl
# Reads:  cross_border-CM-2zone/analysis_data/<design>/mcp_zone_X_<design>.csv
#         cross_border-CM-2zone/Input/weights.csv
# Writes: cross_border-CM-2zone/RO_Settlement/Results/
#
...
#   * Realised corridor rent  CR_corr,t  = (λ_A,t − λ_B,t) · f_BA,t      (eq 3.10)
#   * CCRS allocation     CR^alloc,t = (λ_A,t − λ_B,t) · q_AB        (eq 3.11)
#   * CCRS sufficiency    Ω_AB       = Σ_t W_t · (CR^alloc − Δ)      (eq 3.13)
#
# The 2-zone radial topology has a single corridor (A↔B).  Reporting both
# CR_corr and CR^alloc lets the revenue-adequacy assumption be checked
# directly: in a radial topology with uncongested flow, CR_corr ≈ CR^alloc.
#
# Usage:
#   julia RO_settlement_2zone.jl           # K = 500 €/MWh (base case)
#   julia RO_settlement_2zone.jl 700       # K = 700 €/MWh (sensitivity)
# ─────────────────────────────────────────────────────────────────────────────
...
const ZONES      = ["A", "B"]
const DESIGNS    = ["implicit", "NTC"]   # 2-zone: implicit = fragmented baseline; NTC = coupled (FBMC ≡ NTC in radial)
...
# ─────────────────────────────────────────────────────────────────────────────
# Per-timestep settlement events for a single (design, exporter, importer) pair.
# CR_corr uses the realised energy flow (NetworkManager); CR^alloc uses q.
# ─────────────────────────────────────────────────────────────────────────────
function build_events(design, exporter, importer, k, weights)
    ...
    :NetworkManager in propertynames(imp_df) ||
        error("Zone $importer CSV missing NetworkManager column.")
    ...
    for i in 1:n
        ...
        f_i   = Float64(imp_df.NetworkManager[i])   # import-positive
        ...
        delta    = H_A - Pi_RO
        cr_corr  = (p_imp - p_exp) * f_i
        cr_alloc = (p_imp - p_exp) * q
        omega    = cr_alloc - delta

        # Skip timesteps with no scarcity AND no rent flow (uninteresting).
        if cat == 0 && abs(cr_corr) < 1e-6 && abs(cr_alloc) < 1e-6
            continue
        end
        ...
        push!(rows, (
            ...
            CR_Corridor_EUR_per_h           = cr_corr,
            CR_Alloc_EUR_per_h              = cr_alloc,
            CCRS_Omega_EUR_per_h         = omega,
            ...
            Weighted_CR_Corridor_EUR        = cr_corr  * w,
...
function build_summary(events::DataFrame)::DataFrame
    ...
    for grp in groupby(events, [:Design, :Contract, :Exporter, :Importer])
        ...
        ann_cr_corr  = sum(grp.Weighted_CR_Corridor_EUR)                    / 1e6
        ann_cr_alloc = sum(grp.Weighted_CR_Alloc_EUR)                       / 1e6
        ann_omega    = sum(grp.Weighted_Omega_EUR)                          / 1e6

        coverage_alloc = abs(ann_delta) > 1e-9 ? ann_cr_alloc / ann_delta : NaN
        revenue_check  = abs(ann_cr_alloc) > 1e-9 ? ann_cr_corr / ann_cr_alloc : NaN

        # Shortfall hedging statistics
        ...
        push!(rows, (
            ...
            Annual_CR_Corridor_MEUR             = ann_cr_corr,
            Annual_CR_Alloc_MEUR                = ann_cr_alloc,
            Annual_CCRS_Omega_MEUR           = ann_omega,
            Coverage_Ratio_CRalloc_over_Delta   = coverage_alloc,
            RevenueAdequacy_CRcorr_over_CRalloc = revenue_check,
            ...
            CCRS_Outcome     = ann_omega >  1e-9 ? "CCRS surplus (rent > shortfall)" :
                                  ann_omega < -1e-9 ? "CCRS deficit (rent < shortfall)" :
                                                      "CCRS exactly funds shortfall",
            Hedge_Outcome       = isnan(eur_coverage)          ? "No shortfall" :
                                  eur_coverage >= 1.0 - 1e-6   ? "Fully hedged by CCRS" :
...
function main()
    ...
    println("\nRO Settlement Analysis (2-zone)  |  Strike K = $(STRIKE_K) EUR/MWh")
    println("="^72)
    ...
    for design in DESIGNS
        try
            ...
            println("  [skip] $design — analysis_data not found")
            continue
        end
        ...
        # Revenue adequacy: compare realised corridor rent vs CCRS allocation
        try
            exp_df = load_zone_data(design, "A")
            imp_df = load_zone_data(design, "B")
            if is_net_exporter(design, "A") && is_net_importer(design, "B")
                nT = min(nrow(exp_df), nrow(imp_df), length(weights))
                cr_total = sum(
                    (Float64(imp_df.EOM_price[i]) - Float64(exp_df.EOM_price[i])) *
                    Float64(imp_df.NetworkManager[i]) * Float64(weights[i])
                    for i in 1:nT) / 1e6
                push!(sys_summary, (Design = design, Annual_CR_Total_MEUR = cr_total))
            end
        catch
        end
    end
    ...
    if isempty(all_events)
        println("No cross-border settlement events found across designs.")
        return
    end
    ...
    by_design = combine(groupby(summary, :Design),
        ...
        :Annual_CCRS_Omega_MEUR  => sum => :SumOmega_MEUR,
        :Annual_CR_Corridor_MEUR    => sum => :SumCRCorr_MEUR)
    ...
    println("RO Settlement  |  K = $(STRIKE_K) €/MWh")
    println("─"^130)
    @printf("%-8s  %-8s  %8s  %7s  %7s  %7s  %9s  %9s  %9s  %9s  %9s  %-30s\n",
        "Design","Contract","Cap (MW)","Cat1 h","Cat2 h","Cat3 h",
        "H (M€)","Δ (M€)","CR^corr","CR^a (M€)","Ω (M€)","CCRS outcome")
    println("─"^130)
    for r in eachrow(summary)
        @printf("%-8s  %-8s  %8.0f  %7.0f  %7.0f  %7.0f  %9.3f  %9.3f  %9.3f  %9.3f  %9.3f  %-30s\n",
            r.Design, r.Contract, r.Capacity_MW,
            r.Cat1_Hours_Weighted, r.Cat2_Hours_Weighted, r.Cat3_Hours_Weighted,
            r.Annual_HedgeRequirement_MEUR,
            r.Annual_HedgeShortfall_MEUR,
            r.Annual_CR_Corridor_MEUR,
            r.Annual_CR_Alloc_MEUR,
            r.Annual_CCRS_Omega_MEUR,
            r.CCRS_Outcome)
    end
    println()

    # Per-timestep scarcity events
    sc_events = events[events.Category .> 0, :]
    if !isempty(sc_events)
        println("Per-event detail  (only timesteps with scarcity)")
        println("─"^140)
        @printf("%-8s  %-8s  %3s  %5s  %3s  %8s  %8s  %8s  %12s  %12s  %12s  %12s  %12s\n",
            "Design","Contract","t","Wt(h)","Cat",
            "Pa","Pb","q (MW)",
            "H/h (€)","RO/h (€)","Δ/h (€)","CR^a/h (€)","Ω/h (€)")
        println("─"^140)
        for r in eachrow(sc_events)
            @printf("%-8s  %-8s  %3d  %5.1f  %3d  %8.1f  %8.1f  %8.0f  %12.0f  %12.0f  %12.0f  %12.0f  %12.0f\n",
                r.Design, r.Contract, r.Timestep, r.Weight_h, r.Category,
                r.P_Importer_EUR_per_MWh, r.P_Exporter_EUR_per_MWh, r.Capacity_MW,
                r.HedgeRequirement_EUR_per_h, r.RO_Repayment_EUR_per_h,
                r.HedgeShortfall_EUR_per_h, r.CR_Alloc_EUR_per_h,
                r.CCRS_Omega_EUR_per_h)
        end
        println()
    end

    if !isempty(sys_summary)
        println("Revenue-adequacy check (realised corridor rent vs CCRS allocation)")
        println("─"^90)
        @printf("%-8s  %12s  %12s  %12s  %s\n",
            "Design","CR_corr (M€)","Σ CR^alloc","ratio","interpretation")
        println("─"^90)
        for r in eachrow(revenue_adequacy)
            ratio = haskey(r, :Annual_CR_Total_MEUR) && !ismissing(r.Annual_CR_Total_MEUR) ?
                    r.SumCRCorr_MEUR / (abs(r.SumCRAlloc_MEUR) > 1e-9 ? r.SumCRAlloc_MEUR : NaN) : NaN
            ratio_str = isnan(ratio) ? "—" : @sprintf("%.3f", ratio)
            interp = isnan(ratio) ? "no allocation" :
                     abs(ratio - 1.0) < 0.01 ? "matched (radial expected)" :
                     ratio < 1.0  ? "alloc > realised — over-claim" :
                                    "realised > alloc — surplus rent"
            @printf("%-8s  %12.3f  %12.3f  %12s  %s\n",
                r.Design, r.SumCRCorr_MEUR, r.SumCRAlloc_MEUR, ratio_str, interp)
        end
        println()
    end
end
